---
name: invezgo-data
description: Fetch Indonesian public stock, index, ownership, financial, news, sector, and broker flow data from Invezgo without using the MCP server. Use when the user asks about BEI or IDX stocks, company profiles, stock search, top movers, charts, technical indicators, intraday or order book data, shareholder composition, financial statements, disclosures, corporate actions, indexes, sectors, broker summary, accumulation, inventory, momentum, sankey flow, or specific broker activity on an IDX stock.
---
...
3. Pick the narrowest tool that matches the request.
   Use `search-stock` if the ticker is uncertain.
   Use `information`, `financial`, and `keystat` for company and fundamental questions.
   Use `chart`, `chart-indicator`, `intraday`, `order-book`, `price-diary`, `price-seasonal`, `price-table`, and `time-table` for price action.
   Use `news`, `disclosure`, and `calendar` for catalysts and corporate events.
   Use `list-index`, `index-chart`, `intraday-index`, `sector-stalker`, `sector-rotation`, `top-change`, `top-foreign`, and `top-accumulation` for market-wide analysis.
