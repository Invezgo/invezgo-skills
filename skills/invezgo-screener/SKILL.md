...
description: Screen Indonesian stocks with Invezgo formulas and ownership reports without using the MCP server. Use when the user wants to translate natural-language criteria into an Invezgo screener formula, scan BEI stocks by technical, fundamental, broker, or order-book conditions, or inspect insider activity and holders above one percent or five percent.
---
...
6. Read [formula-cheatsheet.md](references/formula-cheatsheet.md) for the common variables and examples.
7. If the requested function or variable is unusual, read [formula-variables.md](references/formula-variables.md) before composing the formula.
